...
getNewsArticleById: query([text, text], Variant({ Ok: NewsArticle, Err: text }), (title: string, date: string): { Ok: NewsArticle } | { Err: string } => {
...
getCommunityByID: query([text], Variant({ Ok: Community, Err: text }), (id: string): { Ok: Community } | { Err: string } => {
    if (!id) {
...
getCommunityMembers: query([text], Vec(User), (communityId: string): Vec<User> => {
    // Check for valid ID
    if (!communityId) {
    ...
    const principal = Principal.fromText(communityId);
...
getCourseById: query([text], Variant({ Ok: Course, Err: text }), (id:string): { Ok: Course } | { Err: string } => {
  if (!id) {
